Feature: Scaffold TLDR response guidance into baked Pi repos
  ...
  role-workflow enforcement point. The scaffold contract is
  main answer first, then `Summary`, then `TLDR`, with `TLDR` shorter than
  `Summary` and kept as the final section at the bottom.
  ...
  Scenario: Create a new project with the TLDR workflow contract
    ...
    And the system prompt documents the ordered Summary and TLDR footer contract
