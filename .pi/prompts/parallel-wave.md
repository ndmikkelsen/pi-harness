...
- when the request explicitly asks for MCP behavior, route to an MCP-capable path first and only allow shell fallback when MCP is unavailable and that fallback is called out explicitly
